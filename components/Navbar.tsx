  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'h-[70px] bg-navy/90 backdrop-blur-md shadow-lg shadow-navy-dark/50'
    ...
    >
      <nav className="w-full max-w-6xl mx-auto flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-6 lg:gap-8 font-mono text-xs">
          ...
          {/* Resume Link directly to Google Drive */}
      ...
      {/* Mobile Drawer Menu */}
      <div
        className={`fixed inset-y-0 right-0 w-[min(75vw,350px)] bg-navy-light shadow-2xl p-8 flex flex-col justify-center items-center z-50 transform transition-transform duration-300 ease-in-out md:hidden border-l border-navy-lightest ${
          mobileMenuOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
        <button
          onClick={() => setMobileMenuOpen(false)}
          className="absolute top-6 right-6 p-2 text-green hover:text-white"
          aria-label="Close Menu"
        >
          <X className="w-6 h-6" />
        </button>

        <div className="flex flex-col items-center gap-6 font-mono text-sm text-center w-full">
          {navLinks.map((link) => (
            <a
              key={link.id}
              href={link.href}
              onClick={() => setMobileMenuOpen(false)}
              className="text-slate-lightest hover:text-green py-2 transition-colors"
            >
